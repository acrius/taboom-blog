...
И так, воссияем же во имя Юпитера. В смысле создадим наш блог. Одним из преимуществ Gatsby является возможность генерировать проект из шаблонов. И для блога есть хорошоий шаблон https://github.com/gatsbyjs/gatsby-starter-blog.
...
Ну, тут всё просто - переходим в папку `content/blog`. Тут уже есть созданные пробные статьи. Смотрим что внутри, и делаем так же. Создаем директорию `create-blog-with-gatsby`. Кладём внутрь файл `index.md`. Заголовок у markdown файла должен быть такой:
